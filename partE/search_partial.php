...
function generateSearchForm() {
  ...
  if (isset($_GET['enableSession']) && $_GET['enableSession'] == "true") {
    $_SESSION['session'] = true;
    $condition = -1;
  } else if (isset($_GET['enableSession']) && $_GET['enableSession'] == "false") {
    $_SESSION['session'] = false;
    $_SESSION['wineHistList'] = null;
    $condition = -1;
  } 
  if (isset($_SESSION['session']) && $_SESSION['session'] == true) {
    $t->setVariable("urlEndSession", $_SERVER["PHP_SELF"]."?enableSession=false");
    $t->setVariable("urlSearchHistory", "searchHistory.php");
    $t->addBlock("endSession");
  } else {
    $t->setVariable("urlStartSession", "index.php?enableSession=true");
    $t->addBlock("startSession");
  } 
  $regions = getRegion();
